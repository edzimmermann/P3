...
driving_log = append_drinving_log("run_rev") ## This run ran counter clockwize
...
def myGenerator(samples, batch_size=32, camera = -1):
    ...
    ## When no camera is specified we shall mainly use the center
    ## camera and /3 of the time choose a random camera (center, right or left) 
    if (camera < 0):
...
## Crop car hood and sky from image 
## Looking at images
## 160 / 7 = 23 --> 25 
## 160 / 3 = 53,  160/2 = 80  -->  average = 65 or  70 
...
def checkpoint_file_name(model_name):
    return os.path.join(model_name, "weights{epoch:02d}-{val_loss:.3f}")

checkpointer = ModelCheckpoint(filename=checkpoint_file_name(MODEL_NAME), verbose=1, save_best_only=False)
